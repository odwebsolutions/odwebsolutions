      ...
      >
        ...
        {files.length > 0 ? (
          <div className="text-sm text-brand font-medium">
            ...
            <div className="text-[11px] text-white/30 mt-1 font-normal">
...
export function ClientForm() {
  ...
  // Netlify requires the form to POST to the current page URL with multipart/form-data
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Use FormData directly — do NOT convert to URLSearchParams as that strips files
    const data = new FormData(form);

    try {
      const response = await fetch("/client-form", {
        method: "POST",
  ...
  const handleSave = () => {
    ...
    data.forEach((v, k) => {
      // Skip file entries — localStorage can't store them
      if (typeof v === "string") obj[k] = v;
    });
    try {
      localStorage.setItem("od_client_form", JSON.stringify(obj));
      setSaveLabel("✓ Progress Saved");
      setTimeout(() => setSaveLabel("Save Progress"), 2500);
    } catch {
  ...
  if (submitted) {
    return (
      <div className="min-h-screen bg-ink-deep flex items-center justify-center px-5 pt-20">
      ...
      <div className="px-5 lg:px-10 pb-28">
        <div className="max-w-4xl mx-auto">
          {/*
            ...
            - name="client-information" matches the hidden form-name input
            - data-netlify="true" tells Netlify to process this form
            - data-netlify-honeypot="bot-field" enables spam protection
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-4 pt-2">
              ...
              >
                {submitting ? (
                  <>
                    <span className="inline-block h-4 w-4 border-2 border-ink-deep/30 border-t-ink-deep rounded-full animate-spin" />
                    Submitting…
                  </>
                ) : (
